...
账户信息目前**只有 CLI**：页面/接口只**消费** `data/positions/open.json`
（`/api/decision?position=auto`），不直接调交易所。
...
> 🏦 **账户取数没有页面入口，这是刻意的**：取数只在 CLI（§5），页面只**消费**
> `data/positions/open.json` 这一份规范化产物。这样"交易所读数"与"页面展示"之间
> 只有**一个可核验的中间文件**，而不是一条隐式链路 —— 也保证页面**永远不会**因为
> 有人点了一下就去打交易所接口。
