...
```powershell
python run_p2.py --selftest      # ① 全量自检（18 步，含 HTTP 冒烟 + 页面渲染冒烟 + 前端布局验收）
...
python tools/account_read.py --preflight    # 交易前置检查：余额 / 持仓 / 手续费率
```
...
真账户**只读**接入（§5）只做两件事：① 把**真实**余额 / 持仓 / 手续费率读出来
（并如实区分"没有"与"读不到"）；② 做交易前置检查（费率是否还支持门槛 11.34 bp）。
**不代下单、不碰资金** —— 授权用的 Agentic 账户本身**不可提币**。
...
**真实账户实测费率与假设逐项一致**（永续 maker 2 / taker 6 bp、现货 rToken 5 / 5 bp，
见 §5.2）｜
**快照重放回测**（`docs/55`，4.5 天窗口 / 292 笔信号）：过门槛信号在 in_house 窗口按
maker 口径**胜率 84.9%~92.0%、均值 +16.2~+18.2 bp**（跨 3 个交易日、4 个标的稳定），
不过滤基线胜率仅 8.3%~10.7% → **门槛增量 +41~+61 pp**；
同信号下 taker 胜率 **0%**（复现"吃单被成本吃穿"的路线判断）｜
自检 **18 步**（离线）全过、退出码 0。全部可复跑，出处逐条写在 `docs/40`。

**第 4 段 · 完成度**
已完成：执行成本模型（三种方式对比 + 实测联合分布）、四层 agent 链、
可复跑日志（`--log` / `--replay`，契约字段逐字节比对）、独立页面与 API、
公网部署配置、prompt 外部化与事件驱动降本、事件判定校准集、
**真实账户只读接入**（Agentic 账户 OAuth → 只读取数 + 交易前置检查，见 §5）。
未完成（主动写）：无挂单队列位置模型；事件源非毫秒级；联合分布只有三天；
现货腿自 09-14 起零成交；**账户还没有资金**（实测余额 0），所以"缺真实输入"这条
只解决了一半 —— 见 §8 ⑨。
...
| **数据快照** | 58.4 MB / 35 项，**清单可离线核验**（2026-09-23 补入项目一 09-20~09-23 样本后） |
| 反向依赖 | 项目一**不依赖**本项目 |
...
但**现货行一条不丢** —— 现货成交稀少，被截掉会让"最后成交距今"失真、误触发停牌判据）——
`data/SNAPSHOT.md` 逐文件写明处理方式、行数与 SHA256。**这是如实标注，不是借口。**
（截断规则修订的来龙去脉见 `docs/55` §7.3。）
...
| ⑮ | **前端布局验收**（真浏览器：**决策台 + 执行闭环两页**；溢出 / 字面标记 / 截断 / 卡住的占位符） | 只有真渲染 + 量尺寸才发现的排版故障；只量默认页的话**另一页的故障永远不会被发现** |
| ⑯ | **长跑记录器**（汇总统计算法 / 记录格式 / 空与坏数据） | 稳定性与效率的**证据源**自己算错 |
| ⑰ | **文档路径引用**（以 git 入库状态为准 / 已声明的跨仓库引用 / 未声明失效） | 文档里的路径**点不回去**（继承自隔离前的引用最易腐坏；"磁盘上有但没入库"也算坏） |
| ⑱ | 📡 行情通道 + ⚓ 外部锚 + 🏦 **真实账户取数** + 📊 **快照重放回测**（schema 归一 / 落盘守卫 / **口径守卫** / **只读边界** / 盈亏公式与费率对拍） | 采样器**写坏冻结快照**；归一化列与快照不一致；把"休市偏移"当成"折溢价"报出去；**取数工具越界去写**或把未掩码密钥落盘（见 §5.3）；回测的盈亏公式/费率/门槛与项目常量**不一致** |
| ⑲ | 消息面源可用性（`--net`） | 事件源悄悄失效 |
...
> **主路线**是 Bitget **Agentic 账户**（OAuth 授权，**不需要你交 API key**）。
> 它的三个性质正好对上本项目「只辅助、不自动下单」的定位：**资金隔离**
> （只能碰你转进去的额度）、**不可提币**（官方 FAQ 明确）、**OAuth 不落长期密钥**
> （凭证由 MCP 回调自动落盘，不进聊天、不进文档）。撤权方式＝在官网删掉该账户的 API Key。
...
> 📌 ③ 里的 `--base-map` 不是可选项：现货 rToken 符号是 `RHOODUSDT`、永续是 `HOODUSDT`，
> 两条腿必须归到同一个 `HOOD`。而**不能无条件剥 `R` 前缀** —— 真标的里也有 R 开头的
> （如 `RIVN`），剥了会变成 `IVN`，凭空多出一个"标的"。正解是查**权威符号表**
> （可直接用项目一 `data/universe.csv`，带 `base` / `spot_symbol` / `perp_symbol` 三列）。

⚠️ **`--read` 每次新起一个 MCP 进程。** 这不是洁癖：MCP 进程如果在**授权之前**就启动了，
它**不会热加载**新落盘的凭证，私有接口一律报 `ConfigError: Private endpoint requires
API credentials`（它的提示会诱导你去配 `BITGET_API_*` 环境变量，**别照做** ——
那是备选的手动 Key 路线）。自己起一个新进程即可，**不必重开会话**。

🌐 **前置条件两条**（都在本机实测过）：

1. **先完成一次 OAuth 授权**（一次性）。完整步骤与三个坑见
   `docs/52` §8.1.1；凭证由 MCP 回调自动落盘到 `~/.bitget/oauth_token.json`，
   你**不用手填任何 key**。
2. **要能连通 `bitget.com`**：本机实测**直连不通**，走 Clash 代理（`127.0.0.1:7890`）才通。
   代理写在 `~/.workbuddy/mcp.json` 的 `env` 里，且**必须同时带 `NODE_USE_ENV_PROXY=1`**
   —— Node 的 `fetch` **不读** `HTTP_PROXY`，少它连公开行情都拉不到、OAuth 还会静默失败。
   取数工具**从那份配置读启动方式**，所以你只需要改一处。

### 5.2 前置检查实际输出什么

实测（2026-09-22，24 个只读请求全部 `ok`）：

| 项 | 实测值 | 判据 / 含义 |
|---|---|---|
| 授权 | `authorized: true`（apiKey 由服务端**掩码**成 `bg_ff…7fe4`） | OAuth 生效 |
| 余额 | `accountEquity = 0`、`assets: []`、`fundingAssets: []` | 🔴 **账户是空的 → 无法下单**。要演示/实盘得先把资金划进去；**提币与主→子划转按约定由你自己在 Web 手动完成，Agent 不碰资金** |
| 持仓 | `positions.data.list = null` | **明确无持仓**（这是结论，不是"读不到"） |
| 永续费率 | maker 0.0002 / taker 0.0006 = **2 / 6 bp** | 与 `project2/execution_cost.py` 的 `FEE_PERP_MAKER` / `FEE_PERP_TAKER` **逐项一致** |
| 现货 rToken 费率 | 0.0005 / 0.0005 = **5 / 5 bp**（`RHOODUSDT`） | 与 `FEE_SPOT` 一致 |

> 为什么盯死费率：`EDGE_THRESHOLD_BP = 11.34` 的推导前提就是
> 「往返手续费 13.70 bp = 2×(现货 5.0 + 永续 maker 2.0)」。**费率一旦漂，门槛就得重算** ——
> 所以检查器把账户返回的费率与代码常量**逐项对照**，漂了就报警并点明这一点。

**两个此前不知道、但会影响结论的事实**（都已写进工具注释与自检）：
...
### 5.3 只读是**三层**的，不是"我们保证不写"

| 层 | 做法 |
|---|---|
| **权限层** | Agentic 账户**不可提币**（官方 FAQ），且只能碰你转进去的额度 |
| **代码层** | 工具白名单只有三个：`get_auth_status` / `account_overview` / `position`（且只放行 `info`·`history`·`adlRank`）。下单/撤单/提币/划转**整类不在表里**，`position.action=close` **连参数都传不进去**；启动环境里**剥掉所有 `BITGET_*`** |
| **落盘层** | 写文件前先过脱敏守卫：出现 `secretKey` / `passphrase` 之类的键，或 `apiKey` 竟然**没有掩码**，**直接拒写** |

三层在 `python tools/account_read.py --selftest`（**离线**，用真实返回做 fixture）里都有断言；
它已挂进 `run_p2.py --selftest` 的第 ⑱ 步。
...
|---|---|---|
| `ok` | 读到了 N 条 | 逐条列出 |
| `empty` | 读到了、而且**确实是空的** | 「**没有在途持仓**（已确认）」—— 🔴 官方空仓给的是 `list: null`，不是 `[]` |
| `unavailable` | **读不到**（未授权 / 网络不通 / 字段认不出） | 「持仓**读不到**」+ 原因，且退出码 1 |

> 这条不是洁癖：把 `unavailable` 显示成 0 仓位，就是把**缺数据**说成了**安全**。
> 这正是本项目最容易犯、也最不该犯的错（`project2/account_feed.py` 自检里逐条守着）。

⚠️ **仍未校准的一处**（如实写）：**非空持仓的字段名**。当前账户为空，拿不到真实的非空返回，
所以 `--preflight` 遇到非空持仓时会**原样贴出**并明确警告「未校准、请人工核对」，
不按猜出来的字段名硬解读。

端到端已跑通：`--read` → `--ingest` 报 `empty`（真实账户确实没有持仓）。

**页面入口在哪**：网页左侧「**执行闭环**」那一页的最上面，有一块
**🏦 真实账户（只读）** —— 余额 / 持仓 / 逐 symbol 手续费率 / 前置检查结论，
都带**三态徽标**（读到 · 读到且确认 · **读不到**）。
截图存证：`docs/ui-v3/17-account.png`。

⚠️ 它读的是 `data/account/read_latest.json` 这份**本机落盘**的结果：
`/api/account` 只读文件，**页面本身不打交易所**（刷新一万次也不产生一次外部请求）。
"交易所读数 → 页面"之间只有两个可核验的中间文件（账户取数结果、持仓单），
而不是一条隐式链路 —— 这样页面**永远不会**因为有人点了一下就去碰账户。
...
| `docs/55-快照重放回测（胜率的可验证定义）.md` | 📊 **回测证据记录**：在冻结快照上确定性重放（10 标的×双腿×~895 轮×60s），胜率的精确定义、两条口径（taker 下界 / maker 上界）、门槛增量 +53~+64 pp、以及**全部局限**（窗口一天 / 无队列模型 / LLM 不可重放）；配 `tools/backtest_replay.py`（15 项自检）与 `data/backtest/` 逐笔明细 |
| `docs/52-完整执行辅助团队：编制、接口与落地顺序.md` | 要当成**真实可用的完整交易系统**还缺谁：采集官 / 仓位官 / 对账官 / 复核官 / 日报官；交易所接入的红线与签名；**§8 路线定案：主 Agentic 账户（OAuth，不需要你给 key），API Key 作备选**；§9 跨仓库引用约定 |
| `docs/51-怎么用（小白版）与离场问题的边界.md` | **三步上手** · 什么时候能交易（看哪一格）· **什么时候离场：本项目为什么不给**（配图 `docs/使用流程图.png` / `.svg`） |
| `tools/account_read.py`（`--help`） | 🏦 **真实账户只读取数 + 交易前置检查**（见 §5）：`--read` 每次**新起一个 MCP 进程**取数（绕开"长驻进程不热加载凭证"这个坑）；`--preflight` 查余额 / 持仓 / 手续费率，并与 `execution_cost.py` 的费率常量**逐项对照**。只读三层保证 + 脱敏守卫；自检**离线**（用真实返回做 fixture） |
| `project2/account_feed.py`（`--help`） | **账户/仓位接入层（只读）**：把取数结果规范化成项目持仓单格式（`--ingest` + `--base-map`，下游零改动；`--write` 才落盘）。`--plan` 打印"该跑什么"，**模块自己不执行任何命令**。三态语义守在自检里：`ok` / `empty`（🔴 官方空仓是 `list: null`，不是 `[]`）/ `unavailable` |
...
| `/api/account` | 🏦 **真实账户只读摘要**（读 `data/account/read_latest.json`；**不打交易所**，三态严格区分「没有」与「读不到」）|

> 🏦 **账户是"页面只读本地文件"的**：取数本身只在 CLI（§5），页面上那块
> **🏦 真实账户（只读）**（在「执行闭环」页）读的是 `data/account/read_latest.json`
> 这份**本机落盘**的结果，`/api/account` 也只读文件。
> 所以"交易所读数 → 页面"之间只有**两个可核验的中间文件**（账户取数结果、持仓单），
> 而不是一条隐式链路 —— 也保证页面**永远不会**因为有人点了一下就去碰账户。
...
① **现货腿在若干时段零成交**（报价仍在）→ 现货挂单路径**在这些时段不可执行**。
   准确的时间线（逐笔实测，`docs/29` + `docs/55` §7）：**09-14 00:00Z ~ 09-19 06:46Z 零成交**
   （持续 5 天，09-19 恢复）；09-20 / 09-21 正常（4206 / 2946 笔）；
   🔴 **09-22 起再次整天零成交**（该日永续 509,515 笔、现货 0 笔）——
   所以页面上的 `agent:stale_quotes` / `quote_frozen` 是**真判据在起作用**，
   不是系统算错了；
...
⑨ **执行进度官**的输入依赖真实仓位 —— 取数与规范化**已落地**（Agentic 账户 OAuth：
`tools/account_read.py` + `project2/account_feed.py`，见 §5 与 `docs/52` §8），
页面上也能看到账户状态（「执行闭环」页的 🏦 区块），
但实测账户**余额 0、无持仓**（还没划入资金），所以目前仍只能用 `demo` 合成单演示：
**判据可验证，真实输入待资金到位**。这条局限**没有消除**，只是原因从"取数还没实现"
变成了"账户里还没有钱"（提币与主→子划转按约定由你在 Web 手动完成，Agent 不碰资金）；
⑩ 长跑记录里 412 轮是**旧代码**跑的（记录已加 `src_sha16` 代码指纹按版本分组，
带指纹的轮次从 2026-09-20 起重新累积）—— 旧轮次**不能**给当前代码背书；
⑪ **快照重放回测的窗口是 4.5 天**（`docs/55`：2026-09-19~09-23，292 笔信号、
4 个标的、3 个有信号的交易日）—— 它证明的是"门槛有甄别力、吃单必败"的**方向与稳定性**，
仍不支持任何收益承诺；且 maker 口径是**乐观上界**（无队列位置模型，成交率要按
实测联合分布折算）。
