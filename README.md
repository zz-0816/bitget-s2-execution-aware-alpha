...
> **检查方式**：`python tools/doc_ref_check.py`（本仓库存在 / 已声明跨仓库 / 未声明失效
> 三类分开判；能读到项目一仓库时还会**就地核对上游真有那个文件**）。
> 未声明的失效引用会让它**退出码 1** —— "文档里的路径点不回去"是**会失败的**，
> 不是靠人记住的约定。
...
```powershell
python run_p2.py --selftest        # 离线 17 步
...
| ⑰ | **文档路径引用**（本仓库存在 / 已声明的跨仓库引用 / 未声明失效） | 文档里的路径**点不回去**（继承自隔离前的引用最易腐坏） |
| ⑱ | 消息面源可用性（`--net`） | 事件源悄悄失效 |
