...
见 §5.2）｜
**快照重放回测**（`docs/55`）：过门槛信号在 in_house 窗口按 maker 口径
**胜率 84.6%~100%、均值 +12.3~+16.2 bp、最差单笔 −5.7 bp**，
不过滤基线胜率仅 9.8%~13.3% → **门槛增量 +53~+64 pp**；
...
⑪ **快照重放回测的窗口只有一天**（`docs/55`：2026-09-19，15.3h，30 笔信号集中在
1 个标的）—— 它证明的是"门槛有甄别力、吃单必败"的**方向**，样本量不支持任何
统计结论；且 maker 口径是**乐观上界**（无队列位置模型，成交率要按实测联合分布折算）。
