...
```powershell
python run_p2.py --selftest                           # 18 步全部 [OK]，退出码 0（无需网络与 key；实测 408 项 [OK] / 0 项 [!!] / **0 项 [skip]**；项数会随自检增加，**以现场输出为准**）
...
1. **跑一遍自检**：`python run_p2.py --selftest` → 18 步全部通过 / 0 失败（实测 408 项 `[OK]` / 0 项 `[!!]` / 0 项 `[skip]`（含真浏览器布局验收））
