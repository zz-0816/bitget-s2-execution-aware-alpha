...
> ⚠️ **0 · 先核实截止日期** —— 三个来源**互相不一致**，必须自己打开表单读原文。
>
> | 来源 | 说的日期 |
> |---|---|
> | S2 官方手册（gitbook 时间线表两处 + 赛期描述） | **9 月 27 日**（UTC+8） |
> | 同一页的 FAQ 有一处写 | 9/23 |
> | 本仓库 `docs/45` 的记录（据称来自 Google Form） | 9 月 23 日 23:59 |
>
> - [ ] **打开 Google Form 读它自己的原文**（以表单为准；本仓库**不擅自改日期**，只记冲突）
> - [ ] 把核实到的日期**填回 `docs/45` 与本节**，让三处对齐
> - [ ] 按**更早**的那个日期倒排：发 X 帖 → 填表 → 归档（**顺序不能反**）
>
> 起因与原文见 `docs/53` §5。错一天 = 无效提交，所以放在第 0 条。
...
```powershell
python run_p2.py --selftest                           # 18 步全部 [OK]，退出码 0（无需网络与 key；实测 427 项 [OK] / 0 项 [!!] / **0 项 [skip]**；项数会随自检增加，**以现场输出为准**）
...
python tools\doc_ref_check.py                          # 文档路径引用检查（离线；能读到项目一仓库时会就地核对上游）
...
1. **跑一遍自检**：`python run_p2.py --selftest` → **18 步全部通过 / 退出码 0**
   　判据是「**退出码 0 + 0 项 `[!!]`**」，**不要去记某个 `[OK]` 总数** ——
   　每加一个工具它就会涨（同 `docs/53` §4 那条"不要写死"的教训）。
   　参考量级：2026-09-21 实测 **417 项 `[OK]`**、0 项 `[!!]`。
   　⚠️ 在**回收站不可用的环境**（受限沙箱）里，①② 会因 `safe-delete` 报错退出码 1 ——
   　那是环境问题（堆栈之上的断言仍全 `[OK]`），不是代码问题。
