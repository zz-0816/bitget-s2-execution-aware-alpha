...
| 自检 | ✅ `python run_p2.py --selftest`（**18 步离线 + 2 步联网**；实测 397 项 [OK] / 0 项 [!!] / **0 项 [skip]**，含 HTTP 冒烟 + 页面渲染冒烟 + **真浏览器布局验收** + 事件判定回归门槛） |
...
| **外部价格锚** | ✅ `tools/mcp_anchor.py`：用**官方 bitget-mcp-server**（`https://agent.bitget.com/mcp`，无需账号）取真实美股报价，给 rToken 定价一个**外部参照**；口径按开闭市自动降级（开市=折溢价／休市=休市偏移，**不可分离就不冒充**） |
